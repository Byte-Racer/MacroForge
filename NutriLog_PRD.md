# MacroForge — Personal Meal Tracker PRD
...
| Water (ml) | ⭐⭐ Medium | Separate tracker widget |
...
## AI Prompt (use this to start coding)

> Build a personal meal tracking web app called **NutriLog** using React + Vite + Tailwind CSS + React Router + Recharts + Phosphor Icons. Use localStorage for all data persistence — no backend.
>
> **Design:** Dark theme (#0f0f0f base), off-white text, neon green (#a3e635 or similar) as primary accent for protein/calories, coral for fat/warnings, soft blue for water. Bold display font for numbers (use a Google Font like "Space Grotesk" or "DM Mono"). RPG-card-meets-fitness-tracker aesthetic. Clean, minimal, satisfying to use.
>
> **Pages:**
> 1. **Dashboard** — today's macro rings (protein ultra-prominent), water tracker (tap +250ml), quick-log button, recent meals list, streak counter, BMI badge.
> 2. **Log Meal** — modal/page to create a meal (name it, add food items from library by search, set quantity in grams, auto-calculate macros). Save as template option. Meal type selector (Breakfast/Lunch/Dinner/Snack). Add from recent history.
> 3. **Library** — searchable food item list. Each item: name, kcal/100g, protein/100g, carbs/100g, fat/100g, fibre/100g. Pre-seed with 30 common Indian foods (dal, rice, roti, paneer, curd, chicken breast, eggs, banana, etc.). Add/edit/delete items.
> 4. **Insights** — weekly macro bar chart, daily protein vs target, weight trend line, water trend. Use Recharts.
> 5. **Settings** — macro targets (kcal, protein g, carbs g, fat g, fibre g, water ml), height cm, weight log with date stamps, target weight.
>
> **Data model:** Store foods[], meals[], templates[], settings{} in localStorage. meals entries include date, type, name, items (foodId + qty), and computed totals.
>
> **Priority macros to always show prominently:** protein (g), calories (kcal), fibre (g). Others (carbs, fat) shown in detail views.
>
> Start by scaffolding the full project structure, routing, data layer (localStorage helpers), and Dashboard page. Make it look polished from the first screen.

---

*PRD version: 1.0 — May 2025*
